        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[13px] min-w-[620px]">
            <thead><tr className="text-[11px] uppercase tracking-wider text-stone-500 border-b border-stone-800">
              ...
              <th className="text-right px-4 py-2.5">On hand</th><th className="text-right px-4 py-2.5">Min. reorder</th><th className="text-left px-4 py-2.5">Status</th>
            </tr></thead>
            <tbody>
              {rows === null && <tr><td colSpan={5} className="px-4 py-8 text-center text-stone-500">Loading…</td></tr>}
              {rows !== null && filtered.length === 0 && <tr><td colSpan={5} className="px-4 py-8 text-center text-stone-500">Nothing to show.</td></tr>}
              {filtered.map(r => (
                <tr key={r.id} className="border-b border-stone-800/60">
                  <td className="px-4 py-2 text-stone-100 font-medium">{r.name}</td>
                  <td className="px-4 py-2 text-stone-400 font-mono text-[12px]">{r.code || "—"}</td>
